...
async function loadCompanyProfile(companyId, linkElement) {
    ...
    const appReviewBtn = document.getElementById('appReviewBtn'); // Fetch the button element
    if (appReviewBtn) {
        appReviewBtn.style.display = 'none';
        appReviewBtn.href = '#';
    }

    // --- FIX: Use the new singular API path /api/company/{companyId} ---
    const companyProfileApi = `/api/company/${companyId}`;
    
    try {
        const response = await fetch(companyProfileApi);
        
        if (!response.ok) {
            // This block executes for 400, 404, 500, etc. 
            throw new Error(`Failed to load company profile. Status: ${response.status}.`);
        }
        
        const data = await response.json();

        if (data.status === 'success' && data.company_profile) {
            const profile = data.company_profile;
            const contacts = data.contacts || [];

            // Render Profile Details
            companyTitle.textContent = profile.company_name_clean;
            document.getElementById('profileHeader').textContent = profile.company_name_clean;
            ...
            renderRawNames(profile.raw_names || []);
            ...
            // Fallback for a 200 response with a non-success status
        ...
        // Hide the form fields and show the "Select a company" message
        profileContainer.classList.add('hidden');
        initialMessage.classList.remove('hidden');
    } finally {
